...
import ProfilePagePersonalInfo from "../Profilepage";
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setUserData(data);  // Pastikan data pengguna sudah benar
        } else {
    ...
    <div className="navbar w-full bg-[#003266] h-20">
      <div className="container mx-auto flex items-center lg:justify-between px-4 sm:px-6 h-full lg:gap-48">
        ...
        <div className="flex md:hidden">
          ...
          >
            <div
              className={`absolute w-6 h-0.5 bg-white transition-transform duration-500 ease-in-out ${isMenuOpen ? "rotate-45" : "-translate-y-2"}`}
            />
            <div
              className={`absolute w-6 h-0.5 bg-white transition-all duration-500 ease-in-out ${isMenuOpen ? "opacity-0" : "opacity-100"}`}
            />
            <div
              className={`absolute w-6 h-0.5 bg-white transition-transform duration-500 ease-in-out ${isMenuOpen ? "-rotate-45" : "translate-y-2"}`}
            />
        ...
        <div className="hidden md:block mx-auto">
          <Link to="/profile" className="flex items-center">
            {userData ? (
              <img
                src={userData.photo ? userData.photo : ProfilePagePersonalInfo}  // Pastikan foto diambil dari data API
                alt="profile"
        ...
        >
          ...
          <div className="flex justify-center mb-4">
            <Link to="/profile-info" className="flex items-center">
              {userData ? (
                <img
                  src={userData.photo || "https://via.placeholder.com/150"}  // Gambar profil user
                  alt="profile"
